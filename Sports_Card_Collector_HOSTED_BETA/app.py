...
def image_bottom_crop_to_data_url(uploaded_file):
    ...
    crop = image.crop((0, int(height * 0.58), width, int(height * 0.70)))
...
def read_copyright_year(back):
    ...
    content = [
        ...
        {
            "type": "input_image",
            "image_url": image_to_year_data_url(back),
            "detail": "high",
        },
    ]
...
def analyze_card(front, back=None):
    content = [{
        ...
        "text": (
                ...
                "Read the BACK carefully and transcribe the explicit 4-digit copyright year exactly as visibly printed, especially near the copyright/manufacturer line. Do not infer, calculate, correct, or guess this number. If the 4-digit copyright year is not clearly legible, return copyright_year as an empty string. "
                "For the year field, do not infer, calculate, or guess a release year from statistics, biography text, set design, player history, or the copyright year. If the exact release year is not explicitly visible on the card, return year as an empty string. "
    ...
    if back is not None:
       content.append({"type": "input_image", "image_url": image_to_data_url(back), "detail": "low"})
...
with scan_tab:
    ...
    if st.button("🔎 Identify Card", type="primary", disabled=front is None, use_container_width=True):
        try:
            ...
            if float(identity.get("confidence") or 0) >= .95 and first_num:
                identity["card_number"] = first_num
                identity["_card_number_status"] = "high-confidence image read"
            elif (
                vnum
                and first_num
